...
export default function Documents({
  ...
  return (
    <div
      className={`h-full ${
        activeDocuments ? "w-full sm:w-140" : "w-0"
    ...
    >
      {/* ── Header ── */}
      <div className="flex-shrink-0 flex items-center justify-between px-5 sm:px-8 pt-6 sm:pt-8 pb-5 border-b border-[#7d7d8140]">
        <div>
      ...
      <div className="flex-1 overflow-y-auto px-5 sm:px-8 py-5 flex flex-col">
        ...
        {respDocs.length === 0 ? (
          ...
          <form className="flex flex-col gap-y-5">
            {respDocs.map((doc: any, index: number) => (
              <div
                key={index}
                className="w-full flex flex-col rounded-md border border-[#7d7d8130] bg-[#a875800d] hover:border-[#7d7d8160] transition-all duration-200 px-4 sm:px-5 py-4 gap-2"
              >
                <div className="flex items-center justify-between">
                  <label
                    className="text-[12px] sm:text-[13px] text-[#A87580] font-semibold uppercase tracking-widest"
                    htmlFor={doc.id}
                  >
                    Documento {index + 1}
                  </label>
                  {doc.createdAt && (
                    <span className="text-[11px] sm:text-[12px] text-[#7D7D81]">
                ...
                <button
                  className="self-start flex items-center gap-2 text-[12px] sm:text-[13px] py-1.5 px-3 sm:px-4 rounded-md bg-[#3d3439] border border-[#7d7d8140] hover:border-[#A87580] text-[#DCD9DE] transition-all duration-200 cursor-pointer"
        ...
        <div className="mt-10 mb-2">
          <h5 className="text-xl sm:text-2xl text-(--white-color) font-semibold tracking-tight">
            Agregar documentos
          </h5>
          <p className="text-[12px] sm:text-[16px] text-[#7D7D81] mt-1">
