...
)
{
    ...
    return(
        <section className={`flex flex-col items-center justify-end h-full w-full overflow-scroll ${active ? "flex-2 pb-10": ""}`}>
            ...
            <div className={`w-full px-4 sm:px-[15%] lg:px-[28%] flex items-center justify-start flex-col overflow-scroll pb-8 ${active ? "h-full": "h-[65%]"}`}>
                {tasks.map(({id, content, status}, index) =>{
                    return (
                        <div key={index} 
                            ...
                            <input 
                                className={`border-b p-2 ml-3 text-[#dcd9dee3] outline-none w-full pr-7 text-[15px] sm:text-[17.5px] ${(content.trim() === ""  && index !== 0) ? " border-[#9797975e]" : " border-[#979797]"}`}
