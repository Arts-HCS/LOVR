...
type CalendarTask = [string, string] | [string, string, string];
...
export default function BigCalendarComponent({
  ...
  const selectedDayTasks: CalendarTask[] = selectedDate
    ...
    >
      ...
      >
        ...
        <div className="mt-4 flex flex-col gap-2 overflow-y-auto">
          ...
          {selectedDayTasks.map((task: CalendarTask, i: number) => (
            <div
              key={i}
              className={`${
                task.length === 3 ? "bg-[#4b8056]" : "bg-[#595FB1]"
              } text-[#040404] rounded px-2 py-1.5 text-[15px] sm:text-[17px] w-full flex items-center justify-start cursor-pointer`}
              onClick={() => setActive(2)}
            >
      ...
      <div className="w-full h-[full] p-2 sm:p-4 rounded-[10px] relative">
        ...
        <div
          className={`ml-auto mr-2 flex items-center gap-3 sm:gap-5 ${
            active
              ? "fixed top-[8%] right-4 sm:right-15 z-10"
        ...
        <div className="grid grid-cols-7 gap-y-1.5 sm:gap-y-2.5 gap-x-1 sm:gap-x-3 overflow-scroll pb-8">
          ...
          {Array.from({ length: monthDays }).map((_, i) => {
            ...
            const isSelected = selectedDate?.[2] === day;
              ...
              <div
                ...
                className={`
                  ...
                  ${
                    isToday
                      ? `${
                          isSelected ? "bg-[#dbe1e9b8]" : "bg-[#DCD9DE]"
                        } text-gray-800 border-gray-200 hover:bg-[#b0aeb1] hover:text-gray-800`
                      : `${
                          isSelected ? "bg-[#19191be5]" : "bg-[#232325c7]"
                        } text-gray-200 border-[#454a826b] hover:bg-[#19191bc7]`
                  }
                `}
              >
                <p className="text-[11px] sm:text-[15px] absolute top-1 left-1 sm:left-2">
                  {day}
                </p>
                {dayTasks.length > 0 && (
                  <div className="hidden sm:flex flex-col items-start justify-start gap-1.5">
                    {dayTasks.map((task: any, index: number) => (
                      <span
                        ...
                        className={`text-[13px] ${
                          task.length === 3 ? "bg-[#4b8056]" : "bg-[#595FB1]"
                        } text-[#111112] rounded px-1 py-0.5 truncate w-full`}
                      >
                        {task[0]}
                      </span>
                    ))}
