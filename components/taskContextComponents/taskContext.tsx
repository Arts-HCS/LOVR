...
export default function TaskContext({
  ...
  const handleSaveTask = () => {
    ...
    let sortedOpcionesElegidas = opcionesElegidas.sort((a: number, b: number) => a - b);
    ...
    for (let i of sortedOpcionesElegidas) {
      ...
      if (i === 5) {
        ...
        respuesta += `Instrucciones: ${instructions.length > 3 ? instructions : "Sin instrucciones"}. `;
        respuesta += `\nCuerpo: ${body.length > 3 ? body : "Sin cuerpo"}. `;
      } else {
    ...
    const generateAnswer = async (prompt: string) => {
      setTasksGeneradas((prev: any) => [
        ...prev,
        { id: receivedID, status: 0, title: task.title, generated: "" },
      ]);
      ...
      const responseGenerate = await fetch("/api/generateTaskContent", {
        ...
        body: JSON.stringify({ prompt, images: base64Array }),
      });
      ...
      setTasksGeneradas((prev: any) => {
        return prev.map((task: any) =>
          task.id === receivedID ? { ...task, status: 1, generated: answer } : task
        );
      });
      ...
      const saveResponse = await fetch("/api/saveGenerated", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ baseID: receivedID, generated: answer }),
      });
  ...
  return (
    <div className="w-full sm:w-[50%] rounded-xl mr-0 sm:mr-2 flex flex-col items-start justify-start overflow-scroll h-auto sm:h-158 pr-0 sm:pr-8 pb-10 sm:pb-40 mb-6 sm:mb-0">
      <div className="bg-[#46343c90] pb-4 shadow-[0_4px_20px_rgba(0,0,0,0.1)] flex flex-col w-full h-fit rounded-2xl">
        <h5 className="w-full text-[17px] sm:text-[19px] h-fit font-medium text-[#c2939dc0] p-2.5 border-[#a9737f9a] border text-center bg-[#523a44c7] rounded-xl shadow-[0_4px_30px_rgba(0,0,0,0.15)]">
          {task.title}
        </h5>
        <div className="w-full h-fit flex flex-col items-start justify-start pt-4 px-3 gap-2">
          {opciones.map((option: any, index: number) => {
            return (
              <div key={index} className="w-full flex items-center justify-start h-fit gap-x-2">
                <button
                  key={index}
                  onClick={() => {
                    ...
                    if (!opcionesElegidas.includes(parseInt(option.id)) && !(taskSelected === option.id)) {
                      setOpcionesElegidas((prev) => [...prev, parseInt(option.id)]);
                    } else {
                      setOpcionesElegidas((prev) => prev.filter((id: number) => id != option.id));
                    }
                  }}
                  className={`w-[88%] p-2 rounded-xl hover:bg-[#523a44d7] hover:text-[#c2939dde] ${
                    taskSelected == option.id ? "bg-[#523a44d7] text-[#c2939dde]" : ""
                  } transition-all duration-300 hover:shadow-[0_4px_30px_rgba(0,0,0,0.15)] cursor-pointer text-[15px] sm:text-[17px] text-(--white-color) text-left`}
                >
                  {option.titulo}
                </button>
                <button
          ...
          <div className="w-full flex items-center justify-start h-fit gap-x-2">
            <button
              onClick={() => {
                ...
                if (!opcionesElegidas.includes(5) && !(taskSelected === 5)) {
                  setOpcionesElegidas((prev) => [...prev, 5]);
                } else {
                  setOpcionesElegidas((prev) => prev.filter((id: number) => id != 5));
                }
              }}
              ...
              } transition-all duration-200 hover:shadow-[0_4px_30px_rgba(0,0,0,0.15)] cursor-pointer text-[15px] sm:text-[17px] text-(--black-color) text-left`}
            >
              Personalizado
            </button>
            <button
              className={`transition-all h-10 w-11 flex items-center justify-center text-[18px] transform cursor-pointer rounded-xl hover:bg-[#262225] ${
                opcionesElegidas.includes(5)
                  ? "transform scale-110 bg-[#262225] text-[#B28791]"
                  : "bg-[#2b262abf] text-[#d7d5d9e1]"
              }`}
              onClick={() => {
                if (!opcionesElegidas.includes(5)) {
                  setOpcionesElegidas((prev) => [...prev, 5]);
                } else {
                  setOpcionesElegidas((prev) => prev.filter((id: number) => id !== 5));
                }
            ...
            {taskMenu.map((campo: any, index: number) => {
              return (
                <div key={index} className="flex flex-col w-full h-fit mb-3">
                  <label className="text-[15px] sm:text-[16.5px] mb-2" htmlFor={`${index}`}>
                    {campo}
                  </label>
                  <textarea
      ...
      {taskMenu && (
        <form className="w-full h-fit p-4 rounded-xl border border-[#7d7d8184] lovr-background mt-5 shadow-[0_4px_30px_rgba(0,0,0,0.15)]">
          <label className="text-[15px] sm:text-[16.5px]" htmlFor="instructions">
            Instrucciones
          </label>
          <textarea
            id="instructions"
            placeholder="Cuantas sean necesarias..."
            className="mt-1 w-full h-18 outline-none border border-[#efecec2c] text-[#dbd9ddde] text-[14px] sm:text-[15px] shadow-[0_4px_10px_rgba(0,0,0,0.15)] rounded-lg p-2 bg-[#1c1e2185] resize-none mb-4 focus:h-40 hover:h-22 transition-all duration-350"
            ...
            onChange={(e) => {
              setTasksAnswers((prev: any) => {
                ...
                return {
                  ...
                  [receivedID]: {
                    ...currentTaskGroup,
                    [taskSelected]: { ...currentSelection, thirdInput: e.target.value },
                  },
                };
              });
            }}
          ></textarea>
          <label className="text-[15px] sm:text-[16.5px]" htmlFor="body">
            Contexto
          </label>
          <textarea
            className="mt-1 w-full h-20 outline-none border border-[#efecec2c] text-[#dbd9ddde] text-[14px] sm:text-[15px] shadow-[0_4px_10px_rgba(0,0,0,0.15)] rounded-lg p-2 bg-[#1c1e2185] resize-none focus:h-45 hover:h-24 transition-all duration-350"
            name="body"
            id="body"
      ...
      <div>
        <form className="w-full p-4 rounded-2xl bg-[#2D272C] mt-3">
          <label htmlFor="file" className="text-[16px] sm:text-[18.5px] font-medium cursor-pointer">
            Adjuntar foto(s)
          </label>
          <input
      ...
      <div className="w-full h-fit flex items-start justify-start mt-5 gap-3">
        <button
          className="w-[85%] min-h-10 text-(--black-color) bg-[#C7C4C8] cursor-pointer rounded flex items-center justify-center hover:scale-95 transition-all text-[15px] sm:text-[17px]"
